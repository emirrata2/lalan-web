...
export async function generateMetadata({
  ...
  return {
    title: lang === 'en' ? 'Certifications | Lalan MENA' : 'Sertifikalar | Lalan MENA',
    description: lang === 'en'
      ? 'The international quality, safety and compliance certifications held by Lalan Rubbers.'
      : 'Lalan Rubbers\'ın sahip olduğu uluslararası kalite, güvenlik ve uyumluluk sertifikaları.',
    robots: robotsFor(lang),
    alternates: localizedAlternates(lang, '/sertifikalar'),
  };
}
...
export default async function SertifikalarPage({
  ...
  const t = (await getDictionary(lang)).certsPage;
  const CERT_GROUP_KEY: Record<string, keyof typeof t.certGroups> = { 'Kalite & Tıbbi': 'quality-medical', 'Pazar Erişimi': 'market-access', 'Kimyasal & Çevre': 'chemical-env', 'Sosyal & Etik': 'social-ethical' };
  const POL_GROUP_KEY: Record<string, keyof typeof t.policyGroups> = { 'Kalite & Genel': 'quality-general', 'İş Etiği & Hukuk': 'ethics-legal', 'Sosyal & İnsan Hakları': 'social-hr', 'Çevre & Sürdürülebilirlik': 'env-sustain', 'Tesis & Üretim Güvenliği': 'facility-safety' };
  const certGroupLabel = (l: string) => t.certGroups[CERT_GROUP_KEY[l]] ?? l;
  const polGroupLabel = (l: string) => t.policyGroups[POL_GROUP_KEY[l]] ?? l;
  const policyTitle = (code: string, fallback: string) => (t.policies as Record<string,string>)[code] ?? fallback;
  const certScope = (name: string, fallback: string) => (t.certs as Record<string,{scope:string;desc:string}>)[name]?.scope ?? fallback;
  const certDesc = (name: string, fallback: string) => (t.certs as Record<string,{scope:string;desc:string}>)[name]?.desc ?? fallback;
  const certBody = (name: string, fallback: string) => (t.certBodies as Record<string,string>)[name] ?? fallback;
  const policyCode = (code: string) => (t.policyCodes as Record<string,string>)[code] ?? code;
  return (
    <GradientBackground
      ...
      <main id="main-content" className="max-w-7xl mx-auto px-6 md:px-8 pt-36 pb-32">
        ...
        <div className="flex flex-wrap gap-4 mb-16">
          ...
          >
            ...
            <span className="text-sm font-bold" style={{ color: 'rgba(200,212,232,0.7)' }}>{t.badge}</span>
          </div>
          <div
            className="inline-flex items-center gap-3 px-5 py-3 rounded-full"
          ...
          >
            ...
            <span className="text-sm font-bold" style={{ color: 'rgba(200,212,232,0.7)' }}>{t.categoriesLabel}</span>
          </div>
        </div>
          ...
          {[
            ...
            <div key={group} className="mb-8">
              <div className="flex items-center gap-2.5 mb-3">
                ...
                <span className="text-[10px] font-bold uppercase tracking-[0.15em]" style={{ color: 'rgba(172,199,255,0.45)' }}>{polGroupLabel(group)}</span>
                ...
                {items.map((doc) => (
                  ...
                  >
                    ...
                    <span className="flex-1 text-xs font-semibold leading-snug min-w-0 truncate" style={{ color: 'rgba(200,212,232,0.8)' }}>
                      {policyTitle(doc.code, doc.title)}
                    </span>
                    {'lang' in doc && (
