...
export async function generateMetadata({ params }: { params: Promise<{ lang: string; id: string }> }) {
  ...
  return {
    ...
    alternates: localizedAlternates(lang, `/products/${product.id}`),
    openGraph: {
      title: `${product.name} | Lalan Rubbers`,
      description: product.desc,
...
export default async function ProductPage({ params }: { params: Promise<{ lang: string; id: string }> }) {
  const { lang, id } = await params;
  const product = PRODUCTS.find(p => p.id === id);
  if (!product || !isLocale(lang)) notFound();
  const lp = (path: string) => localizePath(path, lang);

  const related = PRODUCTS.filter(p => p.category === product.category && p.id !== product.id).slice(0, 3);

  // offers bilerek yok: B2B teklif usulü çalışıldığı için sitede fiyat yayınlanmıyor,
  // uydurma bir fiyat/stok alanı eklemek Google'ın yapılandırılmış veri kurallarını ihlal eder.
  const productJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'Product',
    name: product.name,
    sku: product.id,
    url: `${SITE_URL}/${lang}/products/${product.id}`,
    description: product.desc,
  ...
  return (
    ...
    >
      ...
      <main id="main-content" className="max-w-7xl mx-auto px-4 md:px-8 pt-20 md:pt-28 lg:pt-32 pb-16 md:pb-24">
