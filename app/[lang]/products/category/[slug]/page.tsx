...
export async function generateMetadata({ params }: { params: Promise<{ lang: string; slug: string }> }) {
  ...
  if (!cat || !isLocale(lang)) return {};
  return {
    title: `${cat.label} Eldivenler | Lalan Rubbers`,
    description: cat.desc,
    robots: robotsFor(lang),
    alternates: localizedAlternates(lang, `/products/category/${slug}`),
  };
}
...
export default async function CategoryPage({ params }: { params: Promise<{ lang: string; slug: string }> }) {
  ...
  return (
    ...
    >
      ...
      <main id="main-content" className="max-w-7xl mx-auto px-6 md:px-8 pt-32 pb-24">
