...
export async function generateMetadata({
  ...
  return {
    title: lang === 'en' ? 'Presentations & Catalogues | Lalan MENA' : 'Sunumlar & Kataloglar | Lalan MENA',
    description: lang === 'en'
      ? 'Lalan Rubbers product technical presentations, general catalogue and documents — download as PDF or request.'
      : 'Lalan Rubbers ürün teknik sunumları, genel katalog ve belgeleri — PDF formatında indirin veya talep edin.',
    robots: robotsFor(lang),
    alternates: localizedAlternates(lang, '/sunumlar'),
  };
}
...
export default async function SunumlarPage({
  ...
  const t = dict.presentationsPage;
  // Kategori etiketi TR label -> catalog.cats üzerinden dile göre çevrilir
  const CAT_MAP: Record<string, keyof typeof dict.catalog.cats> = { 'Gıdaya Uygun': 'food-safe', 'Kimyasal': 'chemical', 'Tek Kullanımlık': 'disposable' };
  const catLabel = (c: string) => dict.catalog.cats[CAT_MAP[c]] ?? c;
  return (
    <GradientBackground
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 pt-6" style={{ borderTop: '1px solid rgba(172,199,255,0.08)' }}>
              <div className="flex-1">
                <p className="font-bold text-white text-sm mb-1">{t.requestPrompt}</p>
